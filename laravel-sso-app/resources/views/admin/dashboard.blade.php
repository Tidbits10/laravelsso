    ...
    <nav class="bg-red-900 text-white shadow-md">
        <div class="max-w-7xl mx-auto px-4 py-4 flex flex-col lg:flex-row gap-4 lg:gap-0 justify-between lg:items-center">
            ...
            <div class="flex flex-wrap gap-3 items-center">
                ...
                <a href="{{ route('admin.export') }}" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 flex items-center gap-2 font-bold shadow transition"><i class="fa-solid fa-download"></i> Export</a>
                <a href="{{ route('admin.scanner') }}" class="bg-black text-white px-4 py-2 rounded hover:bg-gray-800 flex items-center gap-2 shadow transition"><i class="fa-solid fa-qrcode"></i> Scanner</a>
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            ...
            @foreach ($cards as $card)
                <div class="bg-white dark:bg-gray-800 p-4 rounded shadow-md border-l-4 {{ $card['color'] }}">
                    <p class="text-gray-500 dark:text-gray-400 text-xs font-bold uppercase tracking-wider">{{ $card['title'] }}</p>
            ...
            <div class="w-full md:w-2/3">
                ...
                <div class="flex gap-4">
                    <div class="bg-green-50 dark:bg-gray-700 px-4 py-2 rounded border border-green-200 dark:border-gray-600">
                        ...
                        <p class="text-lg font-black dark:text-white">{{ $stats['total'] > 0 ? round((($stats['approved'] + $stats['ready']) / $stats['total']) * 100) : 0 }}%</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded shadow-md border-t-4 border-yellow-500 mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <form action="{{ route('admin.import') }}" method="POST" enctype="multipart/form-data" class="flex items-center gap-4 bg-gray-50 dark:bg-gray-700 p-3 border dark:border-gray-600 rounded w-full md:w-auto">
                @csrf
                        ...
                        <tbody id="queueTable" class="text-sm divide-y divide-gray-100 dark:divide-gray-600">
                            @forelse ($requests as $request)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors data-row">
                                    <td class="py-4 px-6">
                                        <span class="font-black text-gray-800 dark:text-white">REQ-#{{ $request->id }} | {{ $request->user->student_number }}</span><br>
                                        <span class="text-xs font-bold text-red-900 dark:text-red-400 uppercase search-name">{{ $request->user->name }}</span>
                                    </td>
                                    <td class="py-4 px-6">
                                        <span class="font-medium text-gray-800 dark:text-gray-200 block">{{ $request->type }}</span>
                                        <span class="text-xs text-gray-500 dark:text-gray-400 italic block">{{ $request->reason }}</span>
                                        <span class="text-xs font-bold text-blue-600 dark:text-blue-400 mt-1 block"><i class="fa-regular fa-clock mr-1"></i>{{ optional($request->date_requested)->format('M d, Y h:i A') ?? 'Legacy Data' }}</span>
                                    </td>
                                    <td class="py-4 px-6 status-cell">
                                        ...
                                        @endphp
                                        <span class="px-2 py-1 rounded-full text-xs font-bold uppercase tracking-wide {{ $badge }}">{{ $request->status }}</span>
                                    </td>
                                    <td class="py-4 px-6 text-right">
                                        @if ($request->status === 'Pending')
                                            <div class="flex flex-col gap-2 items-end">
                                                <input type="text" id="rem-{{ $request->id }}" placeholder="Remarks..." class="text-xs px-2 py-1.5 border rounded w-48 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                                <div class="flex gap-2">
                                                    <button onclick="processAction('{{ route('admin.requests.approve', $request) }}', '{{ $request->id }}')" class="bg-green-600 text-white px-3 py-1.5 rounded text-xs hover:bg-green-700 font-bold shadow-sm transition">Approve</button>
                                                    <button onclick="processAction('{{ route('admin.requests.reject', $request) }}', '{{ $request->id }}')" class="bg-red-600 text-white px-3 py-1.5 rounded text-xs hover:bg-red-700 font-bold shadow-sm transition">Reject</button>
                                                </div>
                                            </div>
                                        @elseif ($request->status === 'Approved')
                                            <div class="flex flex-col items-end gap-1">
                                                @if ($request->remarks)<span class="text-xs text-gray-500 dark:text-gray-400 max-w-[200px] truncate" title="{{ $request->remarks }}">Admin: {{ $request->remarks }}</span>@endif
                                                <button onclick="processAction('{{ route('admin.requests.ready', $request) }}')" class="bg-purple-600 text-white font-bold px-3 py-1.5 rounded text-xs hover:bg-purple-700 shadow-sm transition mt-1">Mark Ready</button>
                                            </div>
                                        @else
                                            <div class="flex flex-col items-end">
                                                <span class="text-gray-400 font-bold text-xs uppercase tracking-wider">Completed</span>
                                                @if ($request->remarks)<span class="text-xs text-gray-500 dark:text-gray-400 italic mt-1 max-w-[200px] truncate" title="{{ $request->remarks }}">Note: {{ $request->remarks }}</span>@endif
                                            </div>
                                        @endif
                                    </td>
                                </tr>
                    ...
                    <div class="p-6 flex-1 overflow-y-auto">
                        <ul class="space-y-3">
                            @forelse ($queue as $appointment)
                                <li class="p-4 rounded border dark:border-gray-700 {{ $appointment->status === 'Scheduled' ? 'border-yellow-200 bg-yellow-50 dark:bg-gray-700' : 'border-gray-100 bg-gray-50 opacity-60' }} flex justify-between items-center">
                                    <div>
                                        <span class="font-black text-xl text-red-900 dark:text-red-400 block leading-tight">{{ $appointment->queue_number }}</span>
                                        <span class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">{{ $appointment->user->student_number }} | {{ $appointment->time }}</span>
                                    </div>
                                    @if ($appointment->status === 'Scheduled')
                                        <button onclick="processAction('{{ route('admin.appointments.serve', $appointment) }}')" class="bg-red-900 text-white font-bold px-3 py-1.5 rounded text-xs hover:bg-red-800 shadow-sm transition">Serve</button>
                                    @else
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-600">
                        @forelse ($logs as $log)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <td class="py-3 px-6 text-gray-500 dark:text-gray-400 whitespace-nowrap w-48 text-xs font-mono">{{ $log->created_at->format('M d, Y h:i A') }}</td>
                                <td class="py-3 px-6"><span class="bg-gray-200 dark:bg-gray-600 dark:text-gray-200 px-2 py-1 rounded text-xs font-bold">{{ $log->admin_username }}</span></td>
                                <td class="py-3 px-6 font-medium text-gray-700 dark:text-gray-200">{{ $log->action }}</td>
                            </tr>
                        @empty
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        data: [{{ $stats['pending'] }}, {{ $stats['approved'] + $stats['ready'] }}, {{ $stats['rejected'] }}],
