...
<body class="bg-gray-100 min-h-screen font-sans relative" onload="updateClock()">
    ...
    <div class="max-w-5xl mx-auto px-4 py-8">
        ...
        <div class="mb-8 flex justify-between items-end border-b border-gray-300 pb-4">
            <div>
                <h2 class="text-3xl font-black text-gray-800"><span id="greeting">Hello</span>, <?php echo e(auth()->user()->name); ?>!</h2>
                <p class="text-gray-500 font-medium"><?php echo e(auth()->user()->student_number); ?> | <span id="dateDisplay">Loading date...</span></p>
            </div>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md border-t-4 border-yellow-500 mb-8">
            ...
            <form action="<?php echo e(route('student.requests.store')); ?>" method="POST" class="flex flex-col md:flex-row gap-4">
                <?php echo csrf_field(); ?>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md border-t-4 border-red-900 mb-8">
            ...
            <form action="<?php echo e(route('student.appointments.store')); ?>" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <?php echo csrf_field(); ?>
        ...
        <?php if($notifications->count()): ?>
            <div class="mb-6 p-4 bg-yellow-50 border-l-4 border-yellow-500 rounded text-yellow-900 font-bold">
                <i class="fa-solid fa-bell mr-2"></i>You have <?php echo e($notifications->count()); ?> document(s) ready to claim.
            </div>
        <?php endif; ?>

        <h2 class="text-2xl font-black text-gray-800 mb-4 border-l-4 border-red-900 pl-3">Active Requests</h2>
        <div class="flex flex-col gap-6 mb-12">
            <?php $__empty_1 = true; $__currentLoopData = $activeReqs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $request): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                ...
                <div class="bg-white p-6 rounded-lg shadow-md border border-gray-100">
                    <div class="flex justify-between items-start mb-6 gap-4">
                        <div>
                            <h3 class="font-black text-lg text-gray-800"><?php echo e($request->type); ?></h3>
                            <p class="text-sm text-gray-500">Purpose: <?php echo e($request->reason); ?></p>
                        </div>
                        <div class="text-right">
                            <span class="text-xs font-black text-gray-400 block">REQ-#<?php echo e($request->id); ?></span>
                            <span class="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded mt-1 inline-block border border-blue-100">
                                <i class="fa-regular fa-calendar mr-1"></i><?php echo e(optional($request->date_requested)->format('M d, Y h:i A') ?? 'Just now'); ?>
                    ...
                    <div class="relative flex items-center justify-between w-full mt-4">
                        ...
                        <div class="flex flex-col items-center gap-2 bg-white px-2"><div class="w-8 h-8 rounded-full <?php echo e($step1); ?> flex items-center justify-center font-bold text-sm z-10"><i class="fa-solid fa-file-arrow-up"></i></div><span class="text-xs font-bold text-gray-600 uppercase tracking-wider">Submitted</span></div>
                        <div class="flex flex-col items-center gap-2 bg-white px-2"><div class="w-8 h-8 rounded-full <?php echo e($step2); ?> flex items-center justify-center font-bold text-sm z-10"><i class="fa-solid fa-magnifying-glass"></i></div><span class="text-xs font-bold text-gray-600 uppercase tracking-wider">Processing</span></div>
                        <div class="flex flex-col items-center gap-2 bg-white px-2"><div class="w-8 h-8 rounded-full <?php echo e($step3); ?> flex items-center justify-center font-bold text-sm z-10"><i class="fa-solid fa-box-open"></i></div><span class="text-xs font-bold text-gray-600 uppercase tracking-wider">Ready</span></div>
                    </div>

                    <?php if($request->remarks): ?>
                        <div class="mt-6 p-3 bg-blue-50 border-l-4 border-blue-500 text-sm text-blue-800 rounded">
                            <i class="fa-solid fa-circle-info mr-2"></i><strong>Admin Note:</strong> <?php echo e($request->remarks); ?>

                        </div>
                    <?php endif; ?>

                    <?php if($request->qr_image): ?>
                        <div class="mt-6 flex flex-col items-center p-4 bg-gray-50 border rounded-lg">
                            ...
                            <img src="<?php echo e($request->qr_image); ?>" alt="QR Code" onclick="openQRModal('<?php echo e($request->qr_image); ?>', '<?php echo e($request->qr_text); ?>')" class="w-32 h-32 border-4 border-white shadow-md rounded cursor-pointer hover:scale-105 transition transform">
                            <p class="text-xs text-gray-500 mt-2 text-center break-all"><?php echo e($request->qr_text); ?></p>
        ...
        <div class="flex flex-col gap-4 opacity-75">
            <?php $__empty_1 = true; $__currentLoopData = $historyReqs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $request): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200 flex justify-between items-center">
                    <div>
                        <h3 class="font-bold text-gray-700"><i class="fa-solid fa-clock-rotate-left mr-2"></i><?php echo e($request->type); ?></h3>
                        <p class="text-xs text-gray-500"><?php echo e($request->status); ?> - <?php echo e($request->remarks); ?><br><span class="text-blue-500 font-bold"><?php echo e(optional($request->date_requested)->format('M d, Y h:i A') ?? 'Old Record'); ?></span></p>
                    </div>
    ...
    <script>
        ...
        function updatePassword() {
            ...
            fetch('<?php echo e(route('student.password.update')); ?>', {
                method: 'POST',
...
<?php /**PATH C:\Users\elmer\Documents\Codex\2026-06-19\files-mentioned-by-the-user-mid\work\laravel-sso-app\resources\views/student/dashboard.blade.php ENDPATH**/ ?>